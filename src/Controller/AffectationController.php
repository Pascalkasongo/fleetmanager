...
public function new(
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        // Vérifie si le véhicule est déjà affecté sur une période qui se chevauche
        $affectationExistante = $affectationRepository
            ->createQueryBuilder('a')
            ->where('a.vehicule = :vehicule')
            ->andWhere('a.date_debut <= :dateFin')
            ->andWhere('a.date_fin >= :dateDebut')
        ...
        if ($affectationExistante) {
            $this->addFlash(
                'error',
                'Le véhicule sélectionné est déjà affecté à une mission durant cette période.'
            );

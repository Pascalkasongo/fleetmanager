...
final class SuiviController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager,MaintenanceRepository $maintenanceRepository ): Response
    {
        $suivi = new Suivi();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $vehicule = $suivi->getVehicule();

            $derniereMaintenance = $maintenanceRepository->findOneBy(
                ['vehicule' => $vehicule],
                ['date' => 'DESC']
            );
            if ($derniereMaintenance) {
                $prochainEntretien = $derniereMaintenance->getProchainEntretien();

                // Exemple : enregistrer cette valeur dans Suivi
                $suivi->setProchainEntretien($prochainEntretien);
            }
